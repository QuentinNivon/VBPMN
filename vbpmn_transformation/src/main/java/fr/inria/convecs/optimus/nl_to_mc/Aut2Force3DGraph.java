...
public class Aut2Force3DGraph
{
	...
	public void generateForce3DGraphFile() throws FileNotFoundException
	{
		...
		final Pair<HashSet<AutState>, HashSet<AutEdge>> nodesAndEdges = this.autGraph.nodesAndEdges();
		...
		for (AutState autState : nodesAndEdges.getFirst())
		{
			printWriter.print("    {\"");
			printWriter.print(ID_KEYWORD);
			printWriter.print("\": \"");
			printWriter.print(autState.label());
			printWriter.print("\", \"");
		...
		for (AutEdge autEdge : nodesAndEdges.getSecond())
		{
			...
			printWriter.print(COLOR_KEYWORD);
			printWriter.print("\": \"");
			printWriter.print(autEdge.getColor().getClearColor());
			printWriter.print("\"}");
