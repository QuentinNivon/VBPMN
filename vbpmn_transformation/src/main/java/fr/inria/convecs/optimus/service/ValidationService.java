...
public class ValidationService {
	...
	public Response validateVbpmn(@FormDataParam("file1") InputStream fileStream1,
		...
		try {
			...
			String outputDir = Files.createTempDirectory(Paths.get(OUTPUT_PATH), "vbpmn_").toAbsolutePath().toString();
	...
	// TODO:Cleanup
	private Response processRequest(final List<File> fileList, final FormDataMultiPart formData, final String outputDir) {
		...
		try {
			...
			if (fileList.size() > 2 || fileList.size() <= 0) {
				...
				ModelValidator validator = new VbpmnValidator(SCRIPTS_PATH, outputDir);
				String result;
	...
	private File parseAndTransform(File input) {
		try {
			...
			String outputFileName = new StringBuilder().append(input.getParentFile().getAbsolutePath()).append(File.separator)
