...
public class ValidationService {
	...
	private static final String INPUT_PATH = AppProperty.getInstance().getValue("INPUT_PATH");

	private static final String OUTPUT_PATH = AppProperty.getInstance()
			.getValue("OUTPUT_PATH");
	...
	public Response validateVbpmn(@FormDataParam("file1") InputStream fileStream1,
		...
		try {
			...
			File uploadDir = new File(INPUT_PATH);
			File transformDir = new File(OUTPUT_PATH);
			...
			File file1 = new File(INPUT_PATH + File.separator +fileInfo1.getFileName());
			File file2 = new File(INPUT_PATH + File.separator +fileInfo2.getFileName());
	...
	private File parseAndTransform(File input) {
		try {
			...
			String outputFileName = new StringBuilder().append(OUTPUT_PATH)
					.append(File.separator)
					.append(processOutput.getId()).append(".pif").toString();
