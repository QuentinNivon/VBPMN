...
public class FullCLTSBuilder
{
	...
	public AutGraph truncate()
	{
		this.truncateGreen(this.fullCLTS.startNode(), new HashSet<>());
		return this.fullCLTS;
	}
	...
	private void truncateGreen(final AutState currentState,
							   final HashSet<AutState> visitedStates)
	{
		...
		for (AutEdge outgoingEdge : currentState.outgoingEdges())
		{
			...
			{
				outgoingEdge.targetNode().outgoingEdges().clear();
			}
			else
			{
				this.truncateGreen(outgoingEdge.targetNode(), visitedStates);
			}
		}
	}
