...
class Process:
    ...
    def generateScheduler(self, f):
        ...
        f.write(
            ", QueryProcess:any, ProcessResponse:any] (activeflows: IDS, bpmn: BPROCESS) is\n")  # this parameter stores the set of active flows / tokens
        nbflows = len(self.flows)

        f.write("var mergeid:ID in \n")
        f.write("var ident: ID in\n")
        f.write("select \n")
        cter = 1
        for flow in self.flows:
            f.write(flow.ident + "_begin (?ident of ID) ; scheduler [")
            self.dumpFlowsMsgs(f, False)
            f.write(", QueryProcess, ProcessResponse] (add(ident, activeflows), bpmn)\n")
            f.write(" []\n")
            f.write(flow.ident + "_finish (?ident of ID) ; scheduler [")
            self.dumpFlowsMsgs(f, False)
            #check for inclusive
            node = flow.target
            if isinstance(node, OrJoinGateway):
                f.write(", QueryProcess, ProcessResponse] (activeflows, bpmn)\n")
            else:
                f.write(", QueryProcess, ProcessResponse] (remove(ident, activeflows), bpmn)\n")
            cter = cter + 1
            if (cter <= nbflows):
                f.write("[]\n")
        ...
        f.write(
            ...
            "if (is_merge_possible_v2(bpmn,activeflows,mergeid)) then \n"
            "ProcessResponse(!True of Bool);\nscheduler [")
        self.dumpFlowsMsgs(f, False)
        f.write(", QueryProcess, ProcessResponse] (remove_incf(bpmn, activeflows, mergeid), bpmn)\n")
        f.write("else \n")
    ...
    def genLNT(self, name=""):
        ...
        f.write("    scheduler [")
        self.dumpFlowsMsgs(f, False)
        f.write(", QueryProcess, ProcessResponse] (nil, p1()) \n")
        f.write("||\n")
